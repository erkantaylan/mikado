  ...
  return (
    <ul className="space-y-2.5 text-[14px] text-[var(--ink-soft)]">
      ...
      <li className="flex items-center gap-2">
        {dot(medal.locked, <span className="text-[12px] font-bold">2</span>)} Sealed: the badge counts the quests it still requires
      </li>
      ...
      <li className="flex items-center gap-2">{line(stroke.done)} Powered: a fulfilled quest opening one you can do now</li>
      <li className="flex items-center gap-2">{line(stroke.held)} Powered, but its quest still waits on others</li>
      <li className="flex items-center gap-2">{line(stroke.spent)} Spent: between two fulfilled quests</li>
      <li className="flex items-center gap-2">{line(stroke.locked)} Not powered yet: its quest is not fulfilled</li>
      <li className="flex items-center gap-2">{line(stroke.side)} Side quest, hung on its quest</li>
      <li className="flex items-center gap-2">{line(stroke.bridge)} Bridge: the quests between are hidden</li>
      <li className="flex items-center gap-2">
        <span className="h-5 w-9 shrink-0 rounded border-2 border-dashed border-[var(--ink-faint)]" /> Dashed plate: found on the way
      </li>
      <li className="flex items-center gap-2">
  ...
  return (
    <div className="wt-legend space-y-5 text-[14px] text-[var(--ink-soft)]">
      ...
      <Panel title="Top-right: what sort of card?">
        ...
        <p className="mt-2 text-[13px] text-[var(--ink-faint)]">A plain quest has no mark here.</p>
      </Panel>
      ...
      <Panel title="Lines between cards">
        <ul className="space-y-2.5">
          {row(<RailwaySample flow="done" powered />, 'Powered: a fulfilled quest opening one you can do now; a cart of gold heads for it')}
          {row(<RailwaySample flow="held" powered />, 'Powered, but its quest still waits on others')}
          {row(<RailwaySample flow="spent" powered />, 'Spent: between two fulfilled quests')}
          {row(<RailwaySample flow="locked" powered={false} />, 'Not powered yet: planned track, no rails until its quest is fulfilled')}
          {row(<RailwaySample flow="side" powered />, 'Side quest, fulfilled: green track to its quest')}
          ...
          {row(<RailwaySample flow="bridge" powered={false} />, 'Bridge: a tunnel under the hidden quests between')}
        </ul>
      </Panel>
    </div>
  )
...
type DetailsProps = { m: JourneyModel; item: Item; onPick: (id: string) => void; onClose: () => void }

// Every quest named in the details is a way to it: picking one does what picking a side-panel row does.
function QuestRow({ m, i, onPick }: { m: JourneyModel; i: Item; onPick: (id: string) => void }) {
  return (
    <li>
      <button
        onClick={() => onPick(i.id)}
...
function Details(props: DetailsProps) {
  return props.item.crowns ? <JourneyDetails {...props} q={props.item.crowns} /> : <QuestDetails {...props} />
}

/** The details of a journey card: the journey it stands for, its progress and what is left in it. */
function JourneyDetails({ m, item, q, onPick, onClose }: DetailsProps & { q: JourneyCard }) {
    ...
    <section className="quest-details space-y-3 rounded-lg border-2 border-[var(--panel-border)] bg-[var(--plate)] p-3">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          >
            Open journey <ExternalLink size={14} />
          </a>
        </div>
      ...
      <div>
        ...
        <div className="mt-1 text-[13px] text-[var(--ink-soft)]">
          {q.done} of {q.total} quests fulfilled{q.underway > 0 && !over && ` · ${q.underway} underway`}
        </div>
      </div>
      <p className="rounded bg-[var(--chip)] p-2 text-[14px]">
        ...
        {status === 'done'
          ...
          : status === 'cancelled'
            ? 'it was abandoned with that journey.'
            : 'it is fulfilled when that journey is.'}
      </p>
      {q.open.length > 0 && (
        <div>
      ...
      <div>
        ...
        <ul className="space-y-0.5 text-[14px]">
          {opens.length ? opens.map((i) => <QuestRow key={i.id} m={m} i={i} onPick={onPick} />) : <li className="font-semibold">The journey itself: this is its crowning quest.</li>}
        </ul>
      </div>
    </section>
  )
...
function QuestDetails({ m, item, onPick, onClose }: DetailsProps) {
  const status = m.statusOf(item)
  const before = m.needs.filter((n) => n.from === item.id).flatMap((n) => m.byId.get(n.to) ?? [])
  const opens = m.needs.filter((n) => n.to === item.id).flatMap((n) => m.byId.get(n.from) ?? [])
      ...
      <div>
        ...
        <ul className="space-y-0.5 text-[14px]">
          {item.sideOf ? (
            ...
            <li className="font-semibold">The journey itself: this is its crowning quest.</li>
          )}
        </ul>
      </div>
    </section>
...
export type JourneyMapProps = {
  model: JourneyModel
  state?: JourneyState // from the API; the mock has none
  archived?: boolean
  journeyKey?: string // the live journey's key (J7): turns on search (the mock has none)
...
/**
 * A quest's right-click menu, at the cursor and kept on screen. Esc, a click elsewhere or moving the
...
type PanelTab = 'journey' | 'chronicle' | 'legend' | 'glossary'
const panelTabs: PanelTab[] = ['journey', 'chronicle', 'legend', 'glossary']
...
function usePanelTab(): [PanelTab, (t: PanelTab) => void] {
  const [tab, setTab] = useState<PanelTab>(() => {
    try {
      ...
      if (saved === 'log') return 'chronicle' // the tabs' earlier names
      if (saved === 'quest') return 'journey'
...
/** The quest named by `?quest=` — Q142, q-142 or 142 — if it is on this chart. */
function cardFromUrl(m: JourneyModel): string | null {
  const raw = new URLSearchParams(location.search).get('quest')
  if (!raw) return null
  const n = raw.trim().toLowerCase().replace(/^q-?/, '')
...
function JourneyMapInner({ model: m, state, archived, journeyKey, onRetitle, onSetNpc, atlasHref, banner, version, lineTypes }: JourneyMapProps) {
  ...
  // A quest the URL asks for is shown even if it is one the chart hides; what was saved stays.
  const [hide, setHide] = useHide(journeyKey, (h) => (wanted ? revealing(m, wanted, h) : h))
  ...
  // A quest that was selected and then left the journey is no longer selected.
  ...
  // Picking a quest in the side panel selects it and flies the chart to it: to where the layout
  ...
  // The layout fits the chart once it has placed exactly the quests on it; until then a quest just
  ...
  const onLaidOut = useEffectEvent(() => {
    ...
    if ('quest' in next) return centre(next.quest, 1)
  ...
  // Selecting a quest from anywhere shows its details and flies the chart to it. A quest the chart
  // hides is shown first, then flown to once the layout has placed it.
  const focus = (id: string) => {
    setSelected(id)
    setTab('journey')
    if (!hidden.has(id)) return centre(id, 1)
    afterLayout.current = { quest: id }
    setHide(revealing(m, id, hide))
  }
  // A quest clicked on the chart is selected; if it is partly out of view (say, at the
  ...
  // The quest right-clicked on the chart, and where; drawn from the current model so it follows saves.
      ...
      <div data-theme={theme} className="quest-theme flex h-screen flex-col">
        ...
        <div className="flex min-h-0 flex-1">
          <main ref={mapRef} className="relative min-w-0 flex-1">
            ...
            {/* Rendered before it is laid out (invisibly) so the quests can be measured first. */}
              ...
              >
                ...
                <Controls showInteractive={false}>
                  ...
                  <ControlButton
                    ...
                    title="Show or hide finished quests"
            ...
            {menuItem && onSetNpc && <QuestMenu key={menuItem.id} item={menuItem} x={menu!.x} y={menu!.y} onSetNpc={onSetNpc} onClose={closeMenu} />}
          </main>
