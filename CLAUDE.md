...
- `make screenshots` retakes `docs/screenshots` from the demo journeys. Retake them whenever the UI changes visibly, and commit the code first so the version line in the shots is not `-dirty`.
